...
- 'io.gpio_exposed=12 QUOTED: vendor page states "12x IO" (Features) and "Digital
  I/O Pins | 12" (Technical specs table); no enumerated GPIO pin-list/table is published
  in the spec table, so power_out is omitted'
- 'io.gpio_free=10 DERIVED: the vendor''s own labeled pinout diagram (c3_mini_v2.1.0
  silkscreen photo) enumerates exactly 12 header GPIO pads: TX/21, RX/20, 10, 8, 7,
  6, A3/3, A2/2, A1/1, A0/0, A4/4, A5/5 -- matching the "12x IO" spec exactly. Of
  esp32-c3''s soc.reserved_pins, strapping GPIO2 and GPIO8 are among them (2 pins);
  usb_flash_tied GPIO18/19 are not exposed; ESP32-C3 has no PSRAM to consume a pin.
