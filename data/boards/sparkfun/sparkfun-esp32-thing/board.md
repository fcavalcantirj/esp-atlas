...
- Integrated LiPo battery charger confirmed on product page; battery connector presence
  not separately confirmed, so battery_connector is omitted
- ESP32-D0WDQ6-V3 chip (per SparkFun's own board docs); this chip variant has no PSRAM
  per Espressif's ESP32 series ordering table
...
- 'io.gpio_free=17 DERIVED: the vendor''s own graphical datasheet enumerates all 28
  header pads by GPIO number (36,37,38,39,32,33,34,35,25,26,27,14,12,13 on the left
  header; 21,1,3,22,19,23,18,5,15,2,0,4,17,16 on the right), matching the vendor''s
  own "28 GPIO" count exactly. This is the bare ESP32-D0WDQ6-V3 chip (not a WROOM/WROVER
  module), so GPIO37/38 are physically routed and the datasheet''s own legend marks
  GPIO36/37/38/39 all with "*GPIO: Port Input Only" -- two more input-only pins than
  esp32''s soc.reserved_pins (34/35/36/39), which is scoped to the common WROOM module
  that does not break out 37/38. Of esp32''s soc.reserved_pins, strapping 0/2/5/12/15
  are all exposed (5 pins); input_only 34/35/36/39 are exposed (4 pins); usb_flash_tied
...
- field: aka
  ...
  verified: '2026-09-07'
- field: aka
  url: https://github.com/pioarduino/platform-espressif32/blob/32f6bf400b276b0f61cf42c50490a7808ee3457e/boards/esp32thing.json
  verified: '2026-09-07'
