...
ORACLE_SYSTEM_PROMPT = (
    ...
    "esp32-atlas uses three layers: soc (bare silicon: esp32, esp32-s2, esp32-s3, esp32-c3, "
    "esp32-c6, ...) is packaged into a MODULE (a can with flash+antenna: WROOM, WROVER, MINI, "
    "PICO, SOLO), which goes on a board. A module name is NOT a separate chip family: an "
    "ESP32-WROOM / ESP32-WROOM-32 / ESP32-WROVER / ESP32-MINI / ESP32-PICO / ESP32-SOLO module "
    "all CONTAIN the base esp32 soc, so a page that says ESP32-WROOM (with no -S2/-S3/-C-series "
    "suffix) fully supports soc: esp32. An ESP32-S3-WROOM-1 module contains the esp32-s3 soc; "
    "ESP32-C6-WROOM contains esp32-c6; etc. Do NOT reject soc: esp32 merely because the page "
    "names a WROOM/WROVER/MINI/PICO/SOLO module. Separately: if the boards PRIMARY processor "
    "named on the page is a NON-ESP MCU (e.g. STM32, nRF, RP2040) and the ESP chip is only a "
    "secondary/co-processor radio, DO flag that as a mismatch.\n\n"
