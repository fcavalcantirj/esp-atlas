"""pytest wrapper around the REAL-Groq build_guide golden query matrix
(SPEC-build-guide.md §7).

Every other test around build_guide injects a fake/dead LLM -- see
test_build_guide.py's own docstring. This file is the deliberate exception:
it drives the SAME golden matrix (apps/core/tests/data/build_guide_golden.py,
also used by scripts/build_guide_oracle.py) against REAL inference, either a
...
of the blocking CI job. Run it on demand:
    ...
    make build-guide-oracle   # same matrix, a readable PASS/FAIL table instead
"""
...
ORACLE_PATH = REPO_ROOT / "scripts" / "build_guide_oracle.py"


def _load_oracle():
    spec = importlib.util.spec_from_file_location("build_guide_oracle", ORACLE_PATH)
...
def test_golden_query_against_real_inference(entry):
    ...
    _, parsed, board_exists, error = results[0]
    assert error is None, f"inference call failed: {error}"

    failures = _oracle._check(entry, parsed, board_exists)
