...
// /modules: the typed index behind the "Modules" breadcrumb on every module page. Incremental
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const { title, description } = TYPE_INDEX_COPY.module;
  return {
    title,
    description,
    alternates: { canonical: "/modules" },
    // A cold API must not get an empty index cached by a crawler.
    robots: parts.some((p) => p.type === "module") ? undefined : { index: false, follow: true },
    openGraph: { type: "website", siteName: SITE_NAME, title: `${title} · ${SITE_NAME}`, description, url: "/modules", images: [OG_IMAGE] },
    twitter: { card: "summary_large_image", title: `${title} · ${SITE_NAME}`, description, images: [OG_IMAGE.url] },
  };
}

export default function Page() {
  return <PartTypeIndex type="module" />;
}
